...
{
public:
	WarnLight(Scene* scene, const vec3f& color, const vec3f& pos, const vec3f& ori)
		:Light(scene, color), position(pos), orientation(ori) {
		x_max = y_max = z_max = 512;
		x_min = y_min = z_min = -512;
		p_factor = 0;
	}
	virtual vec3f shadowAttenuation(const vec3f& P) const;
	virtual double distanceAttenuation(const vec3f& P) const;
	...
	void set_p_factor(double p) {
		p_factor = p;
	}
	void set_min(double min, char c) {
		switch (c) {
		case 'x': x_min = min; break;
		case 'y': y_min = min; break;
		case 'z': z_min = min; break;
		}
	}

	void set_max(double max, char c) {
		switch (c) {
		case 'x': x_max = max; break;
		case 'y': y_max = max; break;
		case 'z': z_max = max; break;
		}
	}
protected:
	vec3f position;
